<template>
  <div>
    <div id="GradeLine02" style="width: 100%;height: 330px;"></div>
  </div>
</template>

<script>
import { getToschoolallyears2 } from '../../api/request'
const echarts = require('echarts')
export default {
  name: "getToschoolallyears2",
  data() {
    return {
      cityIndex: [],
      gradeType: [],
  ...
  mounted() {
    this.getToschoolallyears2();
  },
  methods: {
    getToschoolallyears2() {
      getToschoolallyears2().then(res => {
    ...
    getEchart(gradeType, cities, one, two, three) {
      this.myChart = echarts.init(document.getElementById('GradeLine02'), 'dark');
      let itemStyle = {
        normal: {
          label: {
            show: true, //开启显示
