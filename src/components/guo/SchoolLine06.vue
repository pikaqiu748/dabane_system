<template>
  <div>
    <div id="chart_line06" style="width: 100%;height: 330px;"></div>
  </div>
</template>

<script>
import { getSuperBigSchoolAll2018 } from '../../api/request'
...
export default {
  ...
  mounted() {
    this.getSuperBigSchoolAll2018();
  },
  methods: {
    getSuperBigSchoolAll2018() {
      getSuperBigSchoolAll2018().then(res => {
    ...
    getEchart(schoolType, city, jiunian, chuji, shiernian, wanquan, xiaoxue, xiaojiao) {
      this.myChart = echarts.init(document.getElementById('chart_line06'), 'dark');
      this.option = {
        title: {
          text: '2018年超大班额学校类型市州分布'
        },
        tooltip: {
          trigger: 'axis'
        },
