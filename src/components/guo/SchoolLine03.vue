<template>
  <div>
    <div id="chart_line03" style="width: 100%;height: 330px;"></div>
  </div>
</template>

<script>
import { getBigSchoolAll2019 } from '../../api/request'
...
export default {
  ...
  mounted() {
    this.getBigSchoolAll2019();
  },
  methods: {
    getBigSchoolAll2019() {
      getBigSchoolAll2019().then(res => {
    ...
    getEchart(schoolType, city, jiunian, chuji, shiernian, wanquan, xiaoxue, xiaojiao) {
      this.myChart = echarts.init(document.getElementById('chart_line03'), 'dark');
      this.option = {
        title: {
          text: '2019年大班额学校类型市州分布'
        },
        tooltip: {
          trigger: 'axis'
        },
