<template>
  <div>
    <div id="GradeLine04" style="width: 100%;height: 330px;"></div>
  </div>
</template>

<script>
import { getSuperBigGradeAll2019 } from '../../api/request'
...
export default {
  ...
  mounted() {
    this.getSuperBigGradeAll2019();
  },
  methods: {
    getSuperBigGradeAll2019() {
      getSuperBigGradeAll2019().then(res => {
    ...
    getEchart(gradeType, cities, one, two, three, four, five, six, seven, eight, nine) {
      this.myChart = echarts.init(document.getElementById('GradeLine04'), 'dark');
      this.option = {
        title: {
          text: '2019年超大班额年级类型市州分布'
        },
        tooltip: {
          trigger: 'axis'
        },
