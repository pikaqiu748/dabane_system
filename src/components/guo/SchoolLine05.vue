<template>
  <div>
    <div id="chart_line05" style="width: 100%;height: 330px;"></div>
  </div>
</template>

<script>
import { getBigSchoolAll2018 } from '../../api/request'
...
export default {
  ...
  mounted() {
    this.getBigSchoolAll2018();
  },
  methods: {
    getBigSchoolAll2018() {
      getBigSchoolAll2018().then(res => {
    ...
    getEchart(schoolType, city, jiunian, chuji, shiernian, wanquan, xiaoxue, xiaojiao) {
      this.myChart = echarts.init(document.getElementById('chart_line05'), 'dark');
      this.option = {
        title: {
          text: '2018年大班额学校类型市州分布'
        },
        tooltip: {
          trigger: 'axis'
        },
