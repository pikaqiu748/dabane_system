<template>
  <div>
    <div id="chart_line01" style="width: 90%;height: 330px;"></div>
  </div>
</template>

<script>
import { getBigSchoolAll2020 } from '../../api/request'
...
export default {
  ...
  mounted() {
    this.getBigSchoolAll2020();
  },
  methods: {
    getBigSchoolAll2020() {
      getBigSchoolAll2020().then(res => {
    ...
    getEchart(schoolType, city, jiunian, chuji, shiernian, wanquan, xiaoxue, xiaojiao) {
      this.myChart = echarts.init(document.getElementById('chart_line01'), 'dark');
      this.option = {
        title: {
          text: '2020年大班额学校类型市州分布'
        },
        tooltip: {
          trigger: 'axis'
        },
