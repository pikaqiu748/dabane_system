<template>
  <div>
    <div id="chart_line04" style="width: 100%;height: 330px;"></div>
  </div>
</template>

<script>
import { getSuperBigSchoolAll2019 } from '../../api/request'
...
export default {
  ...
  mounted() {
    this.getSuperBigSchoolAll2019();
  },
  methods: {
    getSuperBigSchoolAll2019() {
      getSuperBigSchoolAll2019().then(res => {
    ...
    getEchart(schoolType, city, jiunian, chuji, shiernian, wanquan, xiaoxue, xiaojiao) {
      this.myChart = echarts.init(document.getElementById('chart_line04'), 'dark');
      this.option = {
        title: {
          text: '2019年超大班额学校类型市州分布'
        },
        tooltip: {
          trigger: 'axis'
        },
