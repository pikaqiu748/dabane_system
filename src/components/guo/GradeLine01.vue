<template>
  <div>
    <div id="GradeLine01" style="width: 100%;height: 330px;"></div>
  </div>
</template>

<script>
import { getBigGradeAll2020 } from '../../api/request'
...
export default {
  ...
  mounted() {
    this.getBigGradeAll2020();
  },
  methods: {
    getBigGradeAll2020() {
      getBigGradeAll2020().then(res => {
    ...
    getEchart(gradeType, cities, one, two, three, four, five, six, seven, eight, nine) {
      this.myChart = echarts.init(document.getElementById('GradeLine01'), 'dark');
      this.option = {
        title: {
          text: '2020年大班额年级类型市州分布'
        },
        tooltip: {
          trigger: 'axis'
        },
