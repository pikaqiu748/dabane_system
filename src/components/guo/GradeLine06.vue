<template>
  <div>
    <div id="GradeLine06" style="width: 100%;height: 330px;"></div>
  </div>
</template>

<script>
import { getSuperBigGradeAll2018 } from '../../api/request'
...
export default {
  ...
  mounted() {
    this.getSuperBigGradeAll2018();
  },
  methods: {
    getSuperBigGradeAll2018() {
      getSuperBigGradeAll2018().then(res => {
    ...
    getEchart(gradeType, cities, one, two, three, four, five, six, seven, eight, nine) {
      this.myChart = echarts.init(document.getElementById('GradeLine06'), 'dark');
      this.option = {
        title: {
          text: '2018年超大班额年级类型市州分布'
        },
        tooltip: {
          trigger: 'axis'
        },
