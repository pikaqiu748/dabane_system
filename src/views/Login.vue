...
<template>
  <div class="login-container">
    ...
    <vue-particles color="#6495ED" :particleOpacity="0.7" :particlesNumber="80" shapeType="circle" :particleSize="4" linesColor="#6495ED" :linesWidth="1" :lineLinked="true" :lineOpacity="0.6" :linesDistance="150" :moveSpeed="3" :hoverEffect="true" hoverMode="grab" :clickEffect="true" clickMode="push">
    </vue-particles>
...
<script>
import bgAnimation from '../components/common/bgAnimation/index.vue'
...
export default {
  ...
  methods: {
    login () {
      ...
      //?account=${this.userName}&password=${this.userPwd}
      instance.get(`/user/login?account=${this.userName}&password=${this.userPwd}`).then(res => {
        // console.log(res)
        if (this.userName && this.userPwd) {
          if (res.data.statusCode == 200) {
            const { id, token, roles } = res.data.data
            Cookies.set("userId", id)
            Cookies.set('token', token)
            Cookies.set('role', roles[0].id)
            this.$router.push('/home')
          }
          if (res.data.statusCode == 704) {
            alert("用户名或者密码错误！！")
          }
        } else {
          alert("用户名或者密码错误！！")
      ...
      // this.$router.push('/home')
      // FirstRequest({
      //     url:'/user/login?account=weixx&password=123'
      // }).then(res =>{
      //     console.log(res)
      //     const { id, token, roles }=res.data.data
      //     Cookies.set("userId",id)
      //     Cookies.set('token',token)
      //     Cookies.set('role',roles[0].id)
      //     localStorage.setItem('my_username',this.userName)
      //     this.$router.push('/home')
      // }).catch(err =>{
      //     console.log('login时出错')
      // })
    }
  }
}
